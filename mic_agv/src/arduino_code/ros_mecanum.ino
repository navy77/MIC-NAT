...
const uint8_t DIR[] = {4,46,6,42}; // GPIO Direction motor driver 
const uint8_t PWM[] = {5,45,7,39}; // GPIO PWM motor driver 
const uint8_t EA[] = {8,37,10,35}; // GPIO Encoder channal A 
const uint8_t EB[] = {9,38,11,36}; // GPIO Encoder channal B 
...
// CMD vel
float vel_x,vel_y,vel_z; // velocity from /cmd_vel
...
ros::Publisher VEL("vel_data", &vel_msg);

// Pub vel_target data
std_msgs::Float32MultiArray vel_t_msg;
ros::Publisher VEL_T("vel_t_data", &vel_t_msg);

// Pub pwm data
std_msgs::Float32MultiArray pwm_msg;
ros::Publisher PWM("pwm_data", &pwm_msg);

// Pub ir data
std_msgs::Int32MultiArray ir_msg;
ros::Publisher IR("ir_data", &ir_msg);
...
void enc_array(int eo,int e1,int e2,int e3){
